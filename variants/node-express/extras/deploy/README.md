...
- **The image is built from the repo's `Dockerfile`** (compiled `dist/` on a slim
  Node runtime, running as a non-root user). Tag it with the commit SHA, never
  `latest`, so a rollback is just a redeploy of a known-good tag.
