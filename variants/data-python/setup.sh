#!/usr/bin/env bash
# setup.sh - wire the data-science Python/uv toolchain after `devblueprint init`.
#
...
echo "Wiring the data-science toolchain..."
...
[tool.ruff.lint]
select = ["E", "F", "I", "B", "UP", "SIM", "NPY", "PD"]

[tool.mypy]
strict = true
python_version = "$PY_VERSION"
# Notebooks and third-party data libs are noisy under strict mode; scope mypy to
# the library code where types earn their keep.
files = ["src", "tests"]
...
# nbstripout clears notebook outputs so diffs stay reviewable and data never
# leaks into git; nbqa runs ruff over the notebooks.
...
echo "Verify the gate: ruff check . && ruff format --check . && nbqa ruff notebooks && mypy src && pytest"
