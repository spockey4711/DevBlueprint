...
| [`fly.toml`](fly.toml) | Fly.io | `fly deploy` from the repo - a common Phoenix target. |
...
- **Secrets never live in these files.** Set them in the platform's secret store
  ...
  [`../.dockerignore`](../.dockerignore)). `SECRET_KEY_BASE` and `DATABASE_URL`
  are secrets - they belong in the store, never in a committed file.
- **The image is built from the repo's `Dockerfile`** (an OTP `mix release` in a
  slim Debian runtime). Tag it with the commit SHA, never `latest`, so a rollback
  is just a redeploy of a known-good tag.
- **Migrations are a deliberate release command**, never run on boot - run
  `bin/app eval "App.Release.migrate"` as a release/one-off step (see the runbook).
