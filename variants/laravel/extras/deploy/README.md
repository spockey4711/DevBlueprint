...
|------|--------|----------|
| [`../Dockerfile`](../Dockerfile), [`../docker-compose.yml`](../docker-compose.yml) | Any container runtime | You build and run your own php-fpm image (VPS, Kubernetes, ...). |
...
- **Secrets never live in these files.** Set them in the platform's secret store
  ...
  [`../.dockerignore`](../.dockerignore)). `APP_KEY` is a secret: generate it once
  with `php artisan key:generate --show` and store it in the platform, never here.
- **The image is built from the repo's `Dockerfile`** (php-fpm with the production
  dependencies baked in). Tag it with the commit SHA, never `latest`, so a rollback
