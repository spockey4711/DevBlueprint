...
**Purpose:** the quality bar and how it is enforced for this Nuxt project. This is the
...
```bash
pnpm lint         # ESLint (Nuxt config) - zero warnings
pnpm typecheck    # nuxt typecheck (vue-tsc) - zero errors
...
- **Unit (Vitest + `@nuxt/test-utils` + Vue Testing Library):** `composables/` and `utils/`
  logic (calculations, SEO/meta builders, data adapters and their fallback behavior), and key
  component behavior (guards, reduced-motion branches, parsing). The `nuxt` test environment
  gives auto-imports and runtime config so specs match how the app runs.
- **E2E smoke (Playwright), a few only:** home page renders, primary nav works, a critical
  flow succeeds, a live widget renders its fallback when its `server/api/*` route is unavailable.
- **No snapshot tests of large DOM** - they rot and prove little.

Target: meaningful coverage of `composables/`, `utils/` and critical components, not a global
percentage.

## Tooling

- **ESLint** - `eslint.config.mjs`: `@nuxt/eslint-config/flat` (Vue + TypeScript rules) with
  stylistic rules off so Prettier owns formatting. Run: `pnpm lint`.
- **Prettier** - `prettier.config.mjs` with `prettier-plugin-tailwindcss`. Run: `pnpm format`.
- **Vitest + @nuxt/test-utils** - `vitest.config.ts` (the `nuxt` environment). Specs in
