...
- **The image is built from the repo's `Dockerfile`** (a uv-built virtualenv in a
  slim `python:3.12-slim` runtime running uvicorn). Tag it with the commit SHA,
  never `latest`, so a rollback is just a redeploy of a known-good tag.
- **`.env.example` / `.env.schema` are the single source of truth** for which
  variables every target must supply. `scripts/check-env.sh` (run in CI and the
  gate) fails if they drift apart, so the platform config above and the app stay in
  agreement on what the environment needs.
