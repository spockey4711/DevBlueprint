...
- **The image is built from the repo's `Dockerfile`** (a `bootJar` extracted into a
  JRE runtime). Tag it with the commit SHA, never `latest`, so a rollback is just a
...
- **`.env.example` / `.env.schema` are the single source of truth** for which
  ...
  needs.
- **Run database migrations as a deliberate release step**, not on app boot - a
  Flyway/Liquibase migration job that runs before the new version takes traffic (see
  the runbook's Database section), so the platform config above only serves running
  containers, never mutates the schema on startup.
